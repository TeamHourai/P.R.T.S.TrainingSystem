...
| 权限级别 | 接口范围 |
| --- | --- |
| 公开 | 注册、登录、健康检查、公告、题库只读、公开统计 |
| 登录用户 | 个人资料、正式考试、考试历史、错题、通知状态、答题设置、培训记录 |
| 管理员 | `/api/v1/admin/**`、正式题库与培训题库的新增、修改、删除 |

- 未显式列入矩阵的接口默认拒绝访问。
- 用户身份统一从验证后的 JWT 获取，客户端不提交 `userId` 或管理员操作者 ID。
- 正式发卷接口不返回正确答案和解析；交卷判分后才返回单题结果。
