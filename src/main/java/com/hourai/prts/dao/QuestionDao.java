...
public class QuestionDao {
    ...
    public int insert(Question q) throws SQLException {
        String sql = "INSERT INTO questions (type, difficulty, category, resource, question, options, answer, analysis, has_picture, picture_url, view_count, error_count) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (Connection conn = DriverManager.getConnection(url, user, password);
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    public int update(Question q) throws SQLException {
        String sql = "UPDATE questions SET type=?, difficulty=?, category=?, resource=?, question=?, options=?, answer=?, analysis=?, has_picture=?, picture_url=?, view_count=?, error_count=? WHERE id=?";
        try (Connection conn = DriverManager.getConnection(url, user, password);
             PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, q.getType());
            ps.setInt(2, q.getDifficulty());
